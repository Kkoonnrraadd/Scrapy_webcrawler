...
def get_extracted_data(
    phrase, limit=20, sorting="+price", minumum_price=0, maximum_price=999999999
):
    status_code, json_data = fetch_module.get_response(
        RestApi, phrase, limit, sorting, minumum_price, maximum_price
    )
...
def get_ex_seller_data(
    phase, sellerId
):  # funkcja ktora ma na celu sprawdzenie czy produkt o nazwie phase
...
def write_responses_to_a_list(
    list_of_items_to_search, price_min=0, price_max=1000
):  # dict
    responses_list = []
    ...
    ):  # dla kazdej frazy zwroc wyekstrachowana liste itemow na allegro
        # responses_dict[item] = get_extracted_data(item)
        responses_list.append(
            get_extracted_data(item, minumum_price=price_min, maximum_price=price_max)
        )
...
def run_this_program():
    ...
    first_order_responses = write_responses_to_a_list(
        input_table, price_min=30, price_max=50
    )  # DICT lista bezposrednich odpowiedzi na nasze zapytanie
